...
async function main() {
    ...
    for (var i = 0; i < 3; i++) {
        // Create Proposal
        transaction = await dao.connect(investor1).createProposal(`Proposal ${i + 1}`, "Demo", ether(100), recipient.address)
        await transaction.wait()
    ...
    transaction = await dao.connect(investor1).createProposal(`Proposal 4`, "Demo", ether(100), recipient.address)
    await transaction.wait()
